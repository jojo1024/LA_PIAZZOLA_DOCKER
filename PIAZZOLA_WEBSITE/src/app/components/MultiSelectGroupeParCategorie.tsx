...
interface MultiSelectGroupeParCategorieProps {
  data: any[];
  libelle: string;
  keys: string[];
...
const MultiSelectGroupeParCategorie: React.FC<MultiSelectGroupeParCategorieProps> = ({
  categoriesState,
  setCategoriesState,
  data,
  libelle,
  ...
  const handleChangeCategories = (checked: boolean, item: any) => {
    checked
      ...
      : setCategoriesState(
          categoriesState.filter((i) => i[keys[0]] !== item[keys[0]])
        );
  };
    ...
    <div className={`flex flex-col  ${containerClassname}`}>
      ...
      <Popover className="relative">
        {({ open }) => (
          <>
            <Popover.Button
              className={`flex w-full  justify-between px-4 py-3.5 text-sm rounded-2xl border focus:outline-none select-none ${
                open
                  ? "!border-primary-500 "
                  ...
                  <div className="relative flex flex-col px-5 py-6 space-y-5">
                    {data.map((category) => (
                      <div key={category.label}>
                        <h3 className="font-medium text-left  text-gray-900 dark:text-gray-100 mb-4">
                          {category.label}
                        </h3>
                        {category.data.map((item: any) => (
                          <div key={item[keys[0]]} className="flex items-center mb-4">
                            <Checkbox
                              name={item[keys[0]]}
                              label={`${item[keys[1]]}... ${formatMontantFCFA(item.prixSupplement)}`}
                              onChange={(checked) => handleChangeCategories(checked, item)}
                              defaultChecked={
                                categoriesState.filter((elt) => elt[keys[0]] === item[keys[0]])
                                  .length !== 0
                              }
                            />
                          </div>
...
export default MultiSelectGroupeParCategorie;
